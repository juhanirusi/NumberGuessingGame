...
def user_hint_1():
    ...
    """
    number_ranges = [(1, 10), (11, 20), (21, 30), (31, 40),
    ...
    if user_guess in range(from_number, to_number):
        print(
            f"Not quite. But you're pretty close because the number is between {from_number} and {to_number}.")
    else:
        print(
            f"Nope. Let me give you the first hint, the number is between {from_number} and {to_number}.")
...
while wrong_input:
    if play_game == "no":
        ...
        user_guess = int(
            input("Ok, the computer challenges you to guess a number between 1 and 100: "))
        while too_high_low_input:
            if user_guess > 100:
                user_guess = int(input(
                    "Well now you're going WAY over the roof. I can't take this to the computer when it explicitly said NUMBER BETWEEN 1 AND 100: "))
            elif user_guess < 1:
                user_guess = int(input(
                    "Well now you're just messing with me. I can't take this to the computer when it explicitly said NUMBER BETWEEN 1 AND 100: "))
        ...
        play_game = str(input(
            "Sorry, I can't deliver that message to the computer \U0001F615 , so to (or not to) play the game, simply type 'yes' or 'no': ")).lower()
...
if play_game == 'yes':
    while user_guess != number:
        ...
        if guesses == 1:
            ...
            print(
                f"You got the number right with {guesses} guesses. GOOD JOB!\n")
